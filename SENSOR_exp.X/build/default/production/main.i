...
# 3 "main.c" 2
# 1 "./service/FlashService.h" 1
// FlashService.h
// ----------------------------------------------------------------------------
// Flash memory routines for PIC12F1572
// ----------------------------------------------------------------------------
# 13 "./service/FlashService.h"
...
void main(void)
{
    ...
    (INTCONbits.GIE = 1); // Enable interrupts after everything is configured
    (INTCONbits.PEIE = 1);

    APP_Init(); //Application initialize
    // Restore previous state
       Light_State = Flash_Read(0x0780 // Store sensor state here in Flash);

    while (1) { // Application main loop
        APP_Task();
        Light_State = PORTAbits.RA5;
        Flash_Write(0x0780 // Store sensor state here in Flash, Light_State);
       _delay((unsigned long)((1000)*(2000000/4000.0))); // Sampling interval
    }
